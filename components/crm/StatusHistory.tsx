...
export function StatusHistory({ entries }: { entries: StatusEntry[] }) {
  if (!entries?.length) {
    return (
      <p className="text-sm text-textMuted">Aucune transition enregistrée.</p>
    );
  }
  ...
  return (
    <ol className="relative border-l border-mid pl-5 space-y-4 ml-1.5">
      {sorted.map((e, i) => (
        <li key={i}>
          <span className="absolute -left-[5px] mt-1 h-2.5 w-2.5 rounded-full bg-accent" />
          <div className="text-sm">
            <span className="text-textMuted">{label(e.from)}</span>
            <span className="text-textMuted mx-1.5">→</span>
          ...
          {e.note && (
            <p className="mt-1.5 text-xs text-warmDark/90 italic bg-cream/60 border border-mid rounded-md px-2.5 py-1.5">
              « {e.note} »
            </p>
          )}
        </li>
